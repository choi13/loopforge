...
- **Ollama** ([`providers/ollama.ts`](../packages/core/src/providers/ollama.ts)): the local models only support `completion`, not native tool-calling. So I wrote a **ReAct adapter** ([`providers/react.ts`](../packages/core/src/providers/react.ts)): the system prompt lists the tools and asks the model to emit one tool call as a JSON object; a balanced-brace parser extracts it back into a `ModelTurn`. Any completion model becomes loop-drivable.
